...
P.implementService("haplo:publication:update_branding", function(objectKind, images) {
    return _.map(images, (image, i) => {
        if(!!O.serviceMaybe("haplo:publication:get_image_for_branding", formImagePath(objectKind, i))) {
            if(image.image) {
...
P.webPublication.feature("haplo:publication:branding:update", function(publication, spec) {
    publication.respondToDirectory('/branding',
        function(E, context) {
            ...
            E.response.body = O.serviceMaybe('haplo:publication:get_image_for_branding', formImagePath(imageType, imageIndex));
    });
});